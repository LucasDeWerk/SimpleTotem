...
def ensure_schema() -> None:
    """Migrações incrementais de colunas (criação de tabelas é responsabilidade do instalador)."""
    with engine.begin() as conn:
        ...
        for col in ("senha_simples_enc", "senha_os_enc", "terminal_token", "senha_terminal_enc"):
            if sessao_cols and col not in sessao_cols:
                conn.execute(text(f"ALTER TABLE tconf_api_sessao ADD COLUMN {col} TEXT"))
        if sessao_cols and "terminal_id" not in sessao_cols:
            conn.execute(text("ALTER TABLE tconf_api_sessao ADD COLUMN terminal_id INTEGER"))
